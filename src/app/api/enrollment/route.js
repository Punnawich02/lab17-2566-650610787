...
export const GET = async (request) => {
  ...
  if (studentId) {
    ...
    for( const studentId of studentIdList){
      const std = DB.students.find((x) => x.studentId === studentId)
      students.push(std);
    }
...
export const DELETE = async (request) => {
  ...
  DB.enrollments = DB.enrollments.filter( (std) => std.studentId !== studentId || std.courseNo !== courseNo);
